...
{
    class MacNativeFunctions : NativeFunctions
    {
        ...
        public override IntPtr GetSymbolFromLibrary(IntPtr handle, IntPtr nameValue)
        {
            if (handle == IntPtr.Zero)
            {
                return dlsym(RtldDefault, nameValue);
            }
        ...
        private static readonly IntPtr RtldDefault = dlopen(null);
        ...
        private static extern IntPtr dlopen([MarshalAs(UnmanagedType.LPStr)] string name, int mode = 0x01 | 0x08 /*RTLD_LAZY | RTLD_GLOBAL*/);
