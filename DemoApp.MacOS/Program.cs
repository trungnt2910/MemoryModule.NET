...
namespace DemoApp.MacOS
{
    class Program
    {
        ...
        {
            var asmBytes = File.ReadAllBytes($"SampleDll.dylib");
        ...
        private static NativeAssembly NativeAssembly_AssemblyResolve(object sender, NativeResolveEventArgs e)
        {
            ...
            string secretAsmName = $"Secret.dylib";
            if (e.Name.Equals(secretAsmName, StringComparison.InvariantCultureIgnoreCase))
            {
                using (var resourceStream =
                    Assembly.GetExecutingAssembly().GetManifestResourceStream($"DemoApp.MacOS.{secretAsmName}"))
                {
