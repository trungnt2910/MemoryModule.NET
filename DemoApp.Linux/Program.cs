...
namespace DemoApp.Linux
{
    class Program
    {
        ...
        {
            var asmBytes = File.ReadAllBytes($"SampleDLL{(Environment.Is64BitProcess ? "64" : "")}.so");
        ...
        private static NativeAssembly NativeAssembly_AssemblyResolve(object sender, NativeResolveEventArgs e)
        {
            ...
            string secretAsmName = $"Secret{(Environment.Is64BitProcess ? "64" : "")}.so";
            if (e.Name.Equals(secretAsmName, StringComparison.InvariantCultureIgnoreCase))
            {
                using (var resourceStream =
                    Assembly.GetExecutingAssembly().GetManifestResourceStream($"DemoApp.Linux.{secretAsmName}"))
                {
